...
def generate_social_draft(analyses, regime_result, fg_val, fg_label, review_text=''):
    """用分析数据填充社交动态模板，输出完整文案草稿。review_text: 复盘上条输出的纯文本"""
    now_bj = datetime.now(BJT)
    ...
    btc_near_s = btc.get('near_support', [])
    btc_near_r = btc.get('near_resistance', [])
    eth_near_s = eth.get('near_support', [])
    eth_near_r = eth.get('near_resistance', [])
    regime = regime_result.get('regime', '')
    confidence = regime_result.get('confidence', 0)
    composite = regime_result.get('composite_score', 0)

    o_f = regime_result.get('dimensions', {}).get('order_flow', {})
    o_f_detail = o_f.get('detail', '')
    macro = regime_result.get('dimensions', {}).get('macro_external', {})
    ...
    lines.append(f'\\n🕐 BJ {bj_time_str} | BTC ${btc_p} | ETH ${eth_p} | FG:{fg_val}({fg_label})')
    if composite < -10:
    ...
    # 结构（PA/缠论）— 直接使用 MACD 数值，不依赖 regime_detector 文本
    if btc_macd4 > 50:
    ...
    kline_1d = btc.get('indicators', {}).get('1D', {}).get('candle_label', '')
    kline_4h = btc.get('indicators', {}).get('4H', {}).get('candle_label', '')
    ks = [l for l in [kline_1d, kline_4h] if l and l != '-']
    kline_text = " / ".join(ks) if ks else "待收盘确认"
    lines.append(f'🕯️ K线：{kline_text}')
    # 量价（订单流）— regime_detector detail 字段约定格式: deep-negative/negative/bullish/neutral
    o_f_detail_lower = str(o_f_detail).lower()
    if 'deep-negative' in o_f_detail_lower:
        of_summary = '费率深度负值，空头拥挤=逼空燃料'
    elif 'negative' in o_f_detail_lower:
        of_summary = '费率偏负，空头略占优'
    elif 'bullish' in o_f_detail_lower:
        of_summary = '订单流偏多，主动买入'
    else:
        of_summary = '费率中性，OI平稳'
    lines.append(f'📊 量价：{of_summary}')
    if 'fear' in str(macro_detail).lower():
        if fg_val and fg_val < 25:
            macro_summary = f'FG={fg_val}极度恐惧，散户恐慌=庄家吃货窗口'
        else:
            macro_summary = f'FG={fg_val}恐惧，情绪偏空'
    elif 'greed' in str(macro_detail).lower():
        macro_summary = f'FG={fg_val}贪婪，注意过热'
    else:
    ...
    if eth_entry_f and eth_sl_val and eth_tp_val:
        ...
        eth_rr_str = '?'
    lines.append(f'\\n🎯 BTC 偏多 | 入场{int(btc_entry_f):,} | 止损{btc_sl_val:,} | 止盈{btc_tp_val:,} | RR 1:{btc_rr_str}' + (' ⚠️' if btc_rr_str != '?' and float(btc_rr_str) < 1.0 else ''))
    lines.append(f'🎯 ETH 偏多 | 入场{eth_entry_f:.0f} | 止损{eth_sl_val} | 止盈{eth_tp_val} | RR 1:{eth_rr_str}' + (' ⚠️' if eth_rr_str != '?' and float(eth_rr_str) < 1.0 else ''))
    if fg_val and fg_val < 25:
        lines.append(f'\\n💬 FG={fg_val}极度恐惧——历史上这个位置做空的都成了燃料。')
    else:
        lines.append(f'\\n💬 {regime}不打逆风局，共振方向就是最小阻力线。')
    lines.append('\\n🤖 五系统AI分析，仅供参考')
    return '\\n'.join(lines)
