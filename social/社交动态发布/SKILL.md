---
name: 社交动态发布
description: 发动态文案7步流水线。步骤：同步K线→分析行情→复盘上条→按模板写文案→核验数据→用户审核→配图+保存。与纯分析工作流(加密货币纯分析)是两条独立流程，模板不可混用。
---
...
- **观望 🎯 格式**：`generate_social_draft()` 对 `position='观望'` 的币种输出「🎯 BTC 观望 | 空仓等风」（6/20 确认）
...
### 10.1 gen_charts.py Style 4 崩溃

`python3 scripts/gen_charts.py --style 4 /tmp/social_chart.png` 报错：
```
UnboundLocalError: cannot access local variable 'records' where it is not associated with a value
```
at `gen_charts.py` L356 `for r in records:`。

**变通**：使用 Style 2（仪表盘）替代，Style 2 正常工作。

### 10.2 核验脚本与「观望」冲突（已修复 ✅ 6/20）
...
**修复**（`verify_social_post.py` L123-132）：
1. 读取 `rec['position']`
2. 若含「观望」→ `continue` 跳过该币种的全部 🎯 结构化校验
3. 同时检测文案是否误加了入场/止损/止盈/RR（观望不应有），若有则 ⚠️ 警告
4. 非观望方向仍正常校验价格/RR 容差

### 10.3 publish_social.py 写入时 Timestamp 字段遗漏

`save_social_post.py` 保存记录时 `timestamp` 字段可能为 `None`（如 Post #15），导致后续 `review_last_post.py` 无法确定入场时间来查 1H K 线，返回「无可复盘内容」。

**变通**：手动从文案 🕐 行提取发帖时间，用 DB 查入场后 1H K 线并计算入场→现价变动来补写复盘。
