...
# NOTE: -USDT-SWAP and -USDC-SWAP variants will never match since base symbols
# are extracted by splitting on '-'. They're kept for documentation clarity.
        ...
        for inst_id in batch:
            try:
                ...
                if d.get('code') == '0' and d.get('data'):
                    ...
                    if ts > 0:
                        dates[inst_id] = datetime.fromtimestamp(ts/1000, tz=BJT)
            except Exception:
                pass
    return dates
...
def run():
    ...
    # 费率极端预警（所有扫描到的币，不限合格）
    # BUGFIX: fetch rates for ALL tickers, not just pre_candidates
    all_rates = fetch_funding_rates([t['instId'] for t in tickers if t['volCcy24h'] > 50_000_000])
    extreme_alerts = []
    for t, fr in [(c[0], c[1]) for c in candidates] + [(t, all_rates.get(t['instId'], 0)) for t in tickers if t['volCcy24h'] > 50_000_000 and abs(all_rates.get(t['instId'], 0)) > 0.0005]:
