...
def do_12h_review(review, analysis, now_utc, db):
    ...
    if analysis:
        ...
        is_near_bottom = analysis.get('near_bottom', False) if not spring else False
...
def extract_lessons(dir_score, wyckoff_score, levels_score, pct_change,
    ...
    if analysis:
        ...
        # flat_old format: near_bottom may be nested in sentiment sub-object
        if not has_near_bottom and 'sentiment' in analysis:
            nb = analysis['sentiment'].get('near_bottom', False)
            if isinstance(nb, bool) and nb:
                has_near_bottom = True
...
def do_72h_review(review, analysis, now_utc, db):
    ...
    r12_dir_score = r12_result.get('direction_score', 0)
    if review_6h == '错误' and r12_dir_score == -1:  # dir_score always == r12_dir_score from same result
    ...
    if trend_3d_score == -1:
        if orig_3d_dir == 'bearish' and net_pct > 3:
            ...
            rsi_val = analysis.get('rsi_14') if analysis else None
            rsi = rsi_val if rsi_val is not None else (analysis.get('rsi_4h') if analysis else None)
            fg = ((analysis.get('macro') or {}).get('fear_greed') or analysis.get('fg_val')) if analysis else None
            if rsi is not None and rsi < 20 and fg is not None and fg < 20:
                lessons.append({
                    'category': 'direction_misjudge',
                    'type': '方向误判',
